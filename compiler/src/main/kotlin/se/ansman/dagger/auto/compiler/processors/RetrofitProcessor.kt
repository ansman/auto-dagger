...
import se.ansman.dagger.auto.compiler.models.retrofit.RetrofitModule
...
class RetrofitProcessor<N, TypeName, ClassName : TypeName, AnnotationSpec, F>(
    ...
    private val renderer: Renderer<RetrofitModule<N, ClassName, AnnotationSpec>, F>,
) : Processor<N, TypeName, ClassName, AnnotationSpec> {
    private val validRetrofitAnnotations by lazy {
        Companion.validRetrofitAnnotations.map { environment.renderEngine.className(it) }
    }
    ...
    override fun process(resolver: AutoDaggerResolver<N, TypeName, ClassName, AnnotationSpec>) {
        ...
        resolver.nodesAnnotatedWith(AutoProvideService::class)
            ...
            .map { service ->
                ...
                RetrofitModule(
                    moduleName = environment.renderEngine.rootPeerClass(
                        service.className,
                        environment.renderEngine.simpleNames(service.className).joinToString(
                            prefix = "AutoBindRetrofit",
                            separator = ""
                        )
                    ),
                    installation = HiltModuleBuilder.Installation.InstallIn(targetComponent.className),
                    originatingTopLevelClassName = environment.renderEngine.topLevelClassName(service.className),
                    ...
                    annotation.isAnnotatedWith(Scope::class) -> {
                        ...
                        if (neededScope.className != annotation.className) {
                            logger.error(Errors.Retrofit.invalidScope(
                                ...
                                component = environment.renderEngine.simpleName(targetComponent.className),
