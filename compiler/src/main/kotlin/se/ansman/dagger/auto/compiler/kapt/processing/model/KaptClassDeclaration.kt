...
import se.ansman.dagger.auto.compiler.kapt.processing.isObject
...
data class KaptClassDeclaration(
...
) : KaptDeclaration(), ClassDeclaration<Element, TypeName, ClassName> {
    ...
    override val enclosingDeclaration: KaptClassDeclaration?
        get() = super.enclosingDeclaration as KaptClassDeclaration?

    override val declarations: List<KaptDeclaration> by lazy(LazyThreadSafetyMode.NONE) {
        node.enclosedElements.mapNotNull { it.toKaptDeclaration() }
    }
    ...
    override fun isType(qualifiedName: String): Boolean = qualifiedName.contentEquals(node.qualifiedName)
}