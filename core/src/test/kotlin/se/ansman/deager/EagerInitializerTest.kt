...
class EagerInitializerTest {
    ...
    private val eagerInitializer = EagerInitializer(
        setOf(
            i1,
    ...
    fun `exceptions are rethrown`() {
        ...
        val e = assertFailsWith(IllegalStateException::class) { eagerInitializer.initialize() }
        assertAll {
    ...
    fun `re-entrant calls are not allowed`() {
        lateinit var initializer: EagerInitializer
        val initializable = OrderedInitializable(1) {
            initializer.initialize()
        }
        initializer = EagerInitializer(setOf(initializable))
    ...
    fun `initialize blocks until complete`() {
        ...
        val initializer = EagerInitializer(setOf(OrderedInitializable(1) {
