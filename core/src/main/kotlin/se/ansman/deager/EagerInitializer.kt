...
/** A class that can initialize multiple [Initializable] in order of their [priority][Eager.priority]. */
public class EagerInitializer @Inject constructor(
    ...
    public fun initialize() {
        ...
        synchronized(this) {
            ...
            check(!isInitializing) {
                "Calls to EagerInitializer.initialize is from an initializer is not allowed"
            }
            isInitializing = true
...
internal val Initializable.priority: Int
    get() = if (this is OrderedInitializable) priority else Eager.defaultPriority